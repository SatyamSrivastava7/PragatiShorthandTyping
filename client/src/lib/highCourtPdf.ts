...
function page(attempt: HighCourtAttempt | null, label: string, maximum: number) {
  ...
  const rows = (attempt.alignment || []).map((entry) => {
    ...
    const omitted = entry.status === "missing" ? " omitted" : "";
    return `<span class="${className}${omitted}">${safe(entry.typed || "—")}</span>${expected} `;
  }).join("");
  ...
  const content = `<!doctype html><html><head><title>${safe(result.testSetTitle)} — High Court Report</title>
    <style>
      ...
      .comparison { padding:16px; border:1px solid #cbd5e1; background:#f8fafc; line-height:2; font-size:13px; }
       .full { color:#b91c1c; font-weight:700; text-decoration:underline; } .half { color:#a16207; font-weight:700; text-decoration:underline; } .omitted { padding:1px 4px; border-radius:3px; text-decoration:none; } .full.omitted { background:#fee2e2; } .half.omitted { background:#fef3c7; }
