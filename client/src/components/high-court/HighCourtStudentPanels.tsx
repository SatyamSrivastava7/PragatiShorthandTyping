      ...
      {!sets.length ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {sets.map((set) => <Card key={set.id} className="group border-amber-100 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg">
            <CardHeader><div className="mb-3 flex items-center justify-between"><div className="rounded-lg bg-amber-100 p-2"><Award className="h-5 w-5 text-amber-700" /></div><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-700">English</span></div><CardTitle className="leading-snug">{set.title}</CardTitle><CardDescription className="flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{new Date(set.createdAt).toLocaleDateString()}</CardDescription></CardHeader>
    ...
    <>
      <div className="overflow-x-auto rounded-xl border">
        <table className="w-full min-w-[720px] text-sm">
          ...
          <tbody>{results.map((result) => <tr key={result.testSetId} className="border-t hover:bg-slate-50"><td className="p-4 font-semibold">{result.testSetTitle}</td><td className="p-4">{result.studentName}</td><td className="p-4 text-center">{result.typingMarks} / 100</td><td className="p-4 text-center">{result.pitmanMarks} / 100</td><td className="p-4 text-center">{result.shorthandMarks} / 200</td><td className="p-4 text-center font-bold text-amber-800">{result.totalMarks} / 400</td><td className="p-4 text-right"><Button size="sm" variant="outline" onClick={() => setSelected(result)}><Eye className="mr-1.5 h-4 w-4" /> View</Button></td></tr>)}</tbody>
        </table>
      </div>
